...
void timer_callback2(rcl_timer_t * timer_2, int64_t last_call_time){
  ...
  if (timer_2 != NULL) {
    ...
    RCSOFTCHECK(rcl_publish(&publisherPWM, &revoluciones, NULL));
...
void subscription_callback(const void * msgin){  
  ...
  pwm_value += gn;
  pwm_value = constrain(pwm_value + gn, -255, 255); //Ajustar el valor de PWM en base al error
...
void IRAM_ATTR encoder_callback(){
    ...
    */
    ...
    timestamp_2 = micros(); //timestamp: Representa el tiempo en microsegundos entre dos pulsos consecutivos del encoder. 
